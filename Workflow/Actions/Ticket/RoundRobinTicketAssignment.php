...
class RoundRobinTicketAssignment extends WorkflowAction
{
    ...
    public static function applyAction(ContainerInterface $container, $entity, $value = null)
    {
        ...
        if ($entity instanceof Ticket ) {
            ...
            $minAssigndAgent = array_keys($final, min($final));

            if (count($minAssigndAgent) == $agentCount ) {
                $minAssigndAgent = array_shift($minAssigndAgent);
            } else {
                $minAssigndAgent = min($minAssigndAgent);
            }

            if ($agent = $entityManager->getRepository('UVDeskCoreFrameworkBundle:User')->findOneBy(array('id' => $minAssigndAgent))) {
